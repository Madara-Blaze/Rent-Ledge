...
import { TenanciesTab } from '@/components/dashboard/tenancies-tab';
...
// Workspace-scoped tabs operate on the whole landlord workspace; tenancy-scoped
// tabs need a selected tenancy and show the tenancy selector.
const WORKSPACE_TABS: { key: Tab; label: string }[] = [
...
const TENANCY_TAB_KEYS = new Set<Tab>(TENANCY_TABS.map((t) => t.key));
...
export function Dashboard() {
  ...
  const [tab, setTab] = useState<Tab>(ws ? 'portfolio' : 'overview');
  ...
  }, [ws, tenantTenancies, reloadTenancies]);
  ...
  // Landlords get workspace tabs first; tenants only ever see tenancy-scoped tabs.
  const tabs = ws ? [...WORKSPACE_TABS, ...TENANCY_TABS, { key: 'reports' as Tab, label: 'Reports' }] : TENANCY_TABS;
  const showSelector = TENANCY_TAB_KEYS.has(tab);
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      ...
      <main className="mx-auto max-w-6xl px-6 py-8">
        {/* tab nav */}
        <nav className="flex flex-wrap gap-1 border-b border-white/10" role="tablist" aria-label="Dashboard sections">
          {tabs.map((t) => (
            <button
              key={t.key}
              role="tab"
        ...
        {/* tenancy selector — only for tenancy-scoped tabs */}
        {showSelector && (
          <div className="mt-6 flex flex-wrap gap-2" role="group" aria-label="Select tenancy">
            {options.length === 0 && (
              <p className="text-sm text-white/40">
                {ws ? 'No tenancies yet — create one in the Tenancies tab.' : 'No tenancies yet.'}
              </p>
            )}
            {options.map((o) => (
              <button
        ...
        <div className="mt-8" role="tabpanel">
          {tab === 'portfolio' && <PortfolioTab landlordId={landlordId} />}
          {tab === 'properties' && <PropertiesTab landlordId={landlordId} />}
