...
export async function apiUpload<T>(path: string, form: FormData, retry = true): Promise<T> {
  if (DEMO) throw new ApiError('File uploads are not available in demo mode.', 400);
  const headers: Record<string, string> = {};
  if (accessToken) headers.Authorization = `Bearer ${accessToken}`; // browser sets the multipart boundary
